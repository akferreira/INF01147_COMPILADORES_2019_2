...
int initial_local_var_position = 0;
...
int insert_new_table_entry(VALOR_LEXICO lexical,ARRAY_DIMENSIONS *vector_dimension){
    ...
    if(top_table->symbol_info == NULL){
        ...
        if(top_table->symbol_info != NULL){
            ...
            if(semantic_stack->depth > 0) top_table->symbol_info->position+=initial_local_var_position;
        ...
        int current_position = top_table->symbol_info->position;
...
SYMBOL_INFO retrieve_symbol(VALOR_LEXICO lexical){
    ...
    while(current_table != NULL){
        ...
        while(current_table != NULL && current_table->symbol_info != NULL){
           ...
           // printf("cmp %s -- %s\n",current_table->symbol_info->name,lexical.value.str_value);
